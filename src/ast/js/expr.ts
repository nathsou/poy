...
function show(expr: Expr, indentLevel: number = 0): string {
    ...
    return match(expr, {
        ...
        Binary: ({ lhs, op, rhs }) =>
            `(${show(lhs, indentLevel)} ${op === '==' ? '===' : op} ${show(
                rhs,
                indentLevel,
            )})`,
        Ternary: ({ cond, then, otherwise }) =>
            `(${show(cond, indentLevel)} ? ${show(then, indentLevel)} : ${show(
                otherwise,
                indentLevel,
            )})`,
        Array: ({ elems }) =>
            `[${elems.map(e => show(e, indentLevel)).join(', ')}]`,
        Closure: ({ args, stmts }) =>
            `(${args
                .map(({ name }) => name.mangled)
        ...
        Call: ({ fun, args }) => {
            ...
            return `${show(fun, indentLevel)}(${args
                .map(e => show(e, indentLevel))
                .join(', ')})`;
        },
