...
export const bitterStmtOf = (sweet: SweetStmt): BitterStmt[] =>
    match(sweet, {
        ...
        Assign: ({ lhs, op, rhs }) => [
            BitterStmt.Assign(bitterExprOf(lhs), op, bitterExprOf(rhs)),
        ],
        While: ({ cond, body }) => [
            BitterStmt.While(bitterExprOf(cond), body.flatMap(bitterStmtOf)),
        ],
        For: ({ name, iterator, body }) => [
            BitterStmt.For(
                name,
                bitterExprOf(iterator.ref),
                body.flatMap(bitterStmtOf),
            ),
        ],
