...
export function jsExprOf(bitter: BitterExpr, scope: JSScope): JSExpr {
    ...
    return match(bitter, {
        ...
        Unary: ({ op, expr }) =>
            JSExpr.Unary({ op, expr: jsExprOf(expr, scope) }),
        Binary: ({ lhs, op, rhs }) =>
            JSExpr.Binary({
        ...
        Block: ({ stmts, ret }) => {
            ...
            return ret
                ? jsExprOf(ret, blockScope)
                : JSExpr.Literal({ literal: Literal.Unit });
        },
        If: () => {
            ...
            if (
                ...
                compiledBranches.every(
                    ({ scope }) => scope.statements.length === 0,
                )
            ) {
            ...
            if (!returnsUndefined) {
                ...
                lastStmtFn = (val: JSExpr) =>
                    JSStmt.Assign(resultVar!, '=', val);
            }
        ...
        Tuple: ({ elems }) =>
            JSExpr.Array({ elems: elems.map(expr => jsExprOf(expr, scope)) }),
        Array: ({ elems }) =>
            JSExpr.Array({ elems: elems.map(expr => jsExprOf(expr, scope)) }),
        ...
        VariableAccess: ({ lhs, field }) =>
            JSExpr.Dot(jsExprOf(lhs, scope), field),
    });
}
